...
class RoboTBR:
    ...
    def __reiniciarAngulo(self, anguloReiniciar=0):
        self.robot.reset()
        self.__anguloDefinido = anguloReiniciar
    ...
    def ajustarAnguloIdeal(self, anguloDesejado=None, modo="NORMAL", tolerancia=0.5, maximoTentativas=3):
#    def ajustarAnguloIdeal(self, anguloDesejado=None, modo="NORMAL", tolerancia=0.2, maximoTentativas=3):
#Marilia
    ...
    def zerarConfiguracoes(self, angulo=0, zerarMotoresAuxiliares=True):
        """
        Para o robo e restaura as configuracoes de movimento e angulo.

        Se zerarMotoresAuxiliares for True, os motores central e laterais
        retornam fisicamente para o angulo zero antes de seus encoders serem
        zerados. Os mecanismos devem estar livres para se movimentar.
        """
        # Parar imediatamente a locomocao e os motores das rodas.
        self.robot.stop()
        self.__motorRodaEsquerda.stop()
        self.__motorRodaDireita.stop()

        # Retornar os mecanismos auxiliares para suas posicoes zero.
        if (zerarMotoresAuxiliares):
            motoresAuxiliares = [
                self.motorCentralPrincipal,
                self.motorCentralAuxiliar,
                self.motorLateralDireito,
                self.motorLateralEsquerdo,
            ]
            for motor in motoresAuxiliares:
                if (motor!=None):
                    motor.zerarAngulo()

        # Restaurar as configuracoes padrao do DriveBase.
        self.__velocidadePadraoAndar = 400
        self.__aceleracaoPadraoAndar = 733
        self.__velocidadePadraoGirar = 500
        self.__aceleracaoPadraoGirar = 840
        self.__velocidadeAndarAtual = self.__velocidadePadraoAndar
        self.__aceleracaoAndarAtual = self.__aceleracaoPadraoAndar
        self.__velocidadeGirarAtual = self.__velocidadePadraoGirar
        self.__aceleracaoGirarAtual = self.__aceleracaoPadraoGirar
        self.robot.settings(
            self.__velocidadePadraoAndar,
            self.__aceleracaoPadraoAndar,
            self.__velocidadePadraoGirar,
            self.__aceleracaoPadraoGirar,
        )

        # Zerar odometria, giroscopio, posicao e correcoes acumuladas.
        self.__parametroAjusteGirar = 1
        self.__anguloPartida = angulo
        self.__reiniciarAngulo(angulo)
        self.anguloIdeal = angulo
        self.vetPosicaoTapete = [0,0]
        self.__zerarDesvios()

        if (self.__inLog):
            print("Configuracoes de motores e angulo zeradas com sucesso!")
    ...
    def __andar(self, distancia, velocidade, distanciaMaxima, sensorGiro, ajusteErro, riscoStalled, tratamentoStalled, wait, stop):
        ...
        # Zera apenas a distancia percorrida e preserva a referencia angular.
        # reset() sem parametros tambem zeraria o giroscopio, fazendo o robo
        # perder o angulo acumulado entre um deslocamento e o giro seguinte.
        anguloAtual = self.__leituraAngulo(False)
        self.robot.reset(distance=0, angle=anguloAtual)
                ...
                if (riscoStalled==False):
                    ...
                    while(self.robot.done()==False):
                        ...
                        if (distanciaAndada==distancia):
                            if (stopwatch.time()==tempoAndar+1000):#marilia tempoandar+2000
                                print("Stallled")
                        else:
    ...
    def __girarDrive(self, graus, sensorGiro, variacaoPermitida=1.5, maximoTentativas=3, inLog=True):
...
#Marilia
        ...
        # O comando recebido pela estrategia e relativo, mas o movimento deve
        # considerar a posicao real do robo. Assim, eventuais desvios ocorridos
        # durante os deslocamentos nao sao carregados para o proximo giro.
        hubInicial = self.__leituraAngulo(False)
        anguloMovimentar = (self.anguloIdeal - hubInicial + 180) % 360 - 180

        if (inLog):
            print("Antes do giro principal - Hub: " + str(hubInicial) + " Angulo movimentar: " + str(anguloMovimentar))

        # Executa a diferenca angular calculada. Isso mantem a referencia
        # continua sem depender da referencia absoluta interna do DriveBase.
        self.robot.turn(anguloMovimentar*self.__parametroAjusteGirar)

        # Aguarda a acomodacao mecanica antes de medir o resultado.
        wait(100)
        hub = self.__leituraAngulo(False)

        # Calcula sempre o menor caminho entre o angulo atual e o ideal.
        # Isso evita correcoes de quase 360 graus ao cruzar o ponto 0/360.
        variacao = (self.anguloIdeal - hub + 180) % 360 - 180
        tentativas = 0

        if (inLog):
            print("Apos giro principal - Hub: " + str(hub) + " Variacao: " + str(variacao))

        if (abs(variacao) < abs(variacaoPermitida)):
            if (inLog):
                print("Correcao encerrada: dentro da tolerancia de " + str(variacaoPermitida) + " graus")
            self.robot.stop()
            return

        # Salva as configuracoes do giro principal e usa movimentos mais
        # controlados durante as correcoes pequenas.
        configuracoesOriginais = self.robot.settings()
        self.robot.settings(
            self.__velocidadeAndarAtual,
            self.__aceleracaoAndarAtual,
            80,
            200,
        )

        erroAnterior = abs(variacao)

        while (abs(variacao) >= abs(variacaoPermitida)) and (tentativas < maximoTentativas):
            ...
            hub = self.__leituraAngulo(False)
            variacao = (self.anguloIdeal - hub + 180) % 360 - 180
            erroNovo = abs(variacao)
            melhora = erroAnterior - erroNovo

            if (inLog):
                print("Hub: " + str(hub) + " Variacao: " + str(variacao) + " Tentativa: " + str(tentativas) + " Melhora: " + str(melhora))

            if (erroNovo < abs(variacaoPermitida)):
                if (inLog):
                    print("Correcao encerrada: tolerancia atingida")
                break

            if (melhora < 0.3):
                if (inLog):
                    print("Correcao encerrada: melhora inferior a 0.3 grau")
                break

            erroAnterior = erroNovo

        if (tentativas>=maximoTentativas) and (abs(variacao)>=abs(variacaoPermitida)):
            if (inLog):
                print("Correcao encerrada: limite de tentativas atingido")

        # Restaura velocidade e aceleracao usadas antes das correcoes.
        self.robot.settings(*configuracoesOriginais)
